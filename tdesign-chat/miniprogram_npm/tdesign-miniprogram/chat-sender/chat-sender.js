...
let ChatSender = class ChatSender extends SuperComponent {
    constructor() {
        super(...arguments);
        this.options = {
            multipleSlots: true,
        ...
        this.methods = {
            onkeyboardheightchange(e) {
                ...
                // 不开启自动顶起，不做处理
                if (!this.data.autoRiseWithKeyboard)
                    return;
            ...
            handleUploadClick(e) {
                ...
                // 禁用状态不显示上传面板
                if (this.data.disabled || status === 'disabled')
                    return;
                // 点击按钮切换面板显示状态
            ...
            handleWechatFileUpload(e) {
                return __awaiter(this, void 0, void 0, function* () {
                    try {
                        // 使用微信小程序的选择文件API
                        const res = yield wx.chooseMessageFile({
                            count: 5,
                            // 最多5个文件
                            type: 'all', // 所有类型文件
                        });
                        if (res.tempFiles && res.tempFiles.length > 0) {
                            const files = res.tempFiles.map((item) => (Object.assign(Object.assign({}, item), { 
                                // 其他属性保留
                                url: item.path })));
                            const newFiles = [...this.data.files, ...files];
                            this.setData({ files: newFiles });
